...
class TxnBatchRollbackTask : public TxnTask {
 public:
  TxnBatchRollbackTask(const ClientStub& stub, const std::vector<std::string>& keys,
                       std::shared_ptr<Transaction::TxnImpl> txn_impl, const bool is_one_pc = false)
      : TxnTask(stub), keys_(keys), is_one_pc_(is_one_pc), txn_impl_(txn_impl) {}
